    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">{{isset($udata) ? 'Edit':'Register'}} Blog</h4>
            </div><!-- end card header -->
            <div class="card-body">
                @if ($errors->any())
                ...
                <div class="live-preview">
                    <form action="{{ isset($udata) ? route('admin.blog.update', $udata->id) : route('admin.blog.store') }}" method="POST" enctype="multipart/form-data">
                        @if (isset($udata))
                        @method('patch')
                        @endif
                        @csrf
...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">Manage Blog</h4>
            </div><!-- end card header -->
            <div class="card-body">
                <table class="table table-nowrap container">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <img src="{{ asset( $blog->image) }}" class="me-75 bg-light-danger" style="height:60px;width:60px;" />
                            </td>
                            <td>{{ $blog->title ?? '' }}</td>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    @php $cryptid=Crypt::encrypt($blog->id); @endphp

                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                        <li><a class="dropdown-item" href="{{ route('admin.blog.edit', $cryptid) }}">Edit</a></li>
                                        <li>
                                            ...
                                            <form id="delete-form-{{ $cryptid }}" action="{{ route('admin.blog.destroy', $cryptid) }}" method="post" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                        </li>
